...
import { Homework, RulesPicker } from "./Homework";
...
type Screen =
  ...
  | "arithmetic"
  | "story";
export default function App({ onOpenTeachingJournal }: { onOpenTeachingJournal?: () => void }) {
  ...
  const playing = ["tower", "arithmetic", "story", "homework"].includes(screen);
  return (
      ...
      <main id="main-content">
        {!home ? (
          ...
          <>
            ...
            {screen === "homework" && skill && (
              <Homework
                key={`homework-${skill.id}`}
                skill={skill}
                sound={prefs.sound}
                onExit={() => go("explore")}
                onDone={() => onOpenTeachingJournal ? onOpenTeachingJournal() : go("grownups")}
              />
            )}
        ...
        >
          ...
          <div className="how-step">
            ...
            <div>
              ...
              <p>
                After five days away, you can choose a practice level one step
                easier. Your past achievements stay in your journal.
              </p>
            </div>
          </div>
          <Button
  ...
  const applyComeback = async () => {
    ...
    try {
      ...
      if (comeback.applied) {
        ...
        home.skills.forEach((s) =>
          saveLocal(`summit.arithmetic.active.${s.id}`, null),
        );
      }
      await reload();
      setCome(false);
  ...
  return (
    <div className="explore-page">
      ...
      <section className="return-trail" aria-labelledby="return-heading">
        <div className="return-trail-mark" aria-hidden="true">
        ...
        <div className="return-trail-copy">
          ...
          <p>
            After five days away, choose an easier practice level to find your
            footing. Your past achievements stay in your journal.
          </p>
        </div>
        <button
          className="return-trail-action"
          onClick={() =>
            home.comeback.due ? setCome(true) : setComeInfo(true)
          }
        >
          <span>
            {home.comeback.due
              ? "Your return trail is ready"
              : "A little help returning"}
          </span>
          {home.comeback.due ? "Choose my way back" : "How a comeback works"}{" "}
      ...
      {choose && (
        ...
        >
          ...
          <p className="muted">
            Seven little challenges. Choose your hearts, clock and numbers below.
            A wrong answer uses one heart; when they run out, your level stays unlocked.
          </p>
          <div className="practice-rules">
            <span>
              <Icon name="spark" size={17} />
              <span>
                With the bonus clock on, finish in its window for{" "}
                <strong>+50 points</strong>. With it off, take all the time you need.
              </span>
            </span>
            <span>
              <Icon name="mountain" size={17} />
              <span>
                Three correct, independent answers in a row at your level or above
                unlock the next one. Easier rounds stay practice.
              </span>
            </span>
          </div>
          ...
          <RulesPicker skill={choose} />
          <p className="small muted">Rules apply to your next new arithmetic round. A saved round keeps its original rules. Worksheets and stories use their own setup.</p>
          <Button onClick={() => onPractice(choose)}>
            {readLocal<{ round_id?: string } | null>(
              `summit.arithmetic.active.${choose.id}`,
              null,
            )?.round_id
              ? "Resume this trail"
              : "Start this trail"}{" "}
          ...
          <Button secondary onClick={() => onHomework(choose)}>
            Fix Pip's homework <Icon name="spark" />
          </Button>
          <Button
            secondary
      ...
      {(come || comeInfo) && (
        ...
        >
          ...
          <p className="muted">
            {home.comeback.due
              ? `It's been ${home.comeback.days_away} days. We`
              : "After five days away, we"}{" "}
            can ease each practice skill by one level, never below level one, to
            help you find your footing. You choose whether to take the gentler
            trail. Your past achievements stay in your journal.
          </p>
          <p className="small muted">
            Choosing the gentler trail starts fresh practice at that level.
            Answers from unfinished trails stay in your history.
          </p>
          {home.comeback.due && (
            <p>
          ...
          {home.comeback.due && (
            <Button onClick={() => void applyComeback()} disabled={comeBusy}>
              {comeBusy ? "Finding your footing…" : "Take the gentle trail"}{" "}
              <Icon name="leaf" />
            </Button>
          )}
          <Button
            secondary={home.comeback.due}
            disabled={comeBusy}
            onClick={() => {
              setCome(false);
              setComeInfo(false);
          ...
          >
            {home.comeback.due
              ...
              : "Ready for my next little adventure"}
          </Button>
        </Modal>
      )}
    </div>
