...
const sqlite = new DatabaseSync(process.env.DB_FILE ?? path.join(here, "..", "summit.db"));
...
function transaction<Args extends unknown[], Result>(fn: (...args: Args) => Result) {
...
export function setToday(d: string) {
  db.prepare("INSERT INTO settings(key,value) VALUES('today',?) ON CONFLICT(key) DO UPDATE SET value=excluded.value").run(d);
}
export function addDays(iso: string, n: number) { const d = new Date(iso); d.setUTCDate(d.getUTCDate() + n); return d.toISOString().slice(0, 10); }
export function daysBetween(a: string, b: string) { return Math.round((Date.parse(b) - Date.parse(a)) / 86_400_000); }

export function seed() {
  if ((db.prepare("SELECT COUNT(*) n FROM learners").get() as any).n) return;
  setToday("2026-09-15");
  db.prepare("INSERT INTO learners(id,display_name,age,guardian_consent_ai) VALUES('L-1','Sam',8,0)").run();
  const ins = db.prepare("INSERT INTO skill_progress(learner_id,skill_id,level) VALUES('L-1',?,1)");
  for (const s of SKILLS) ins.run(s.id);
}
export function reset() {
  db.exec("DELETE FROM problems; DELETE FROM rounds; DELETE FROM events; DELETE FROM skill_progress; DELETE FROM learners; DELETE FROM settings;");
  seed();
}
